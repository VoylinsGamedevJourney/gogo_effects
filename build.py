#!/usr/bin/env python3
"""
GoGo Effects Builder Script

This script handles the compilation of FFmpeg and the GoGo Effects plugin
...
environment variables:
  GOGO_EFFECTS_MSYS2_DIR       path to the MSYS2 directory where MSYS2 is installed (default: C:\\msys64)
  GOGO_EFFECTS_GIT_PATH        path to the git executable (default: git)
  GOGO_EFFECTS_CROSS_SYSROOT   root of the cross-build tree, used when cross compiling (default: linux: auto-detect, windows: ${GOGO_EFFECTS_MSYS2_DIR}/ucrt64)

environment variable usage:
  windows       set GOGO_EFFECTS_MSYS2_DIR=C:\\msys64 && python3 build.py
  linux         export GOGO_EFFECTS_GIT_PATH=/usr/bin/git && python3 build.py
"""
...
from src import build_ffmpeg, utils
...
def windows_detected() -> ExitCode:
    if not utils.find_program(utils.GIT_PATH):
        ...
        print(
            "If Git is installed at a non-standard path, please set the GOGO_EFFECTS_GIT_PATH environment variable."
        )
    ...
    if utils.is_current_msys2_ucrt64():
        ...
        if not utils.is_msys2_installed():
            ...
            print(
                "If MSYS2 is installed at a custom location, please set the GOGO_EFFECTS_MSYS2_DIR environment variable."
            )
            print("Use --help for more information.")
...
def main() -> ExitCode:
    ...
    print()
    print("v============================v")
    print("| Done building GoGo Effects |")
    print("^============================^")
    print()

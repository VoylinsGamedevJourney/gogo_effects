...
def get_host_and_sysroot(target_platform: str, arch: str) -> tuple[str, Path]:
    ...
    if target_platform == "windows":
        ...
        if _GOGO_EFFECTS_CROSS_SYSROOT:
            return "x86_64-w64-mingw32", Path(_GOGO_EFFECTS_CROSS_SYSROOT)
        ...
        if not mingw_path.exists():
            raise FileNotFoundError(
                "Mingw sysroot not found. Please specify the `GOGO_EFFECTS_CROSS_SYSROOT` environment variable."
            )
    ...
    if arch == "arm64" and CURR_ARCH != "arm64":
        return "aarch64-linux-gnu", Path(
            _GOGO_EFFECTS_CROSS_SYSROOT
        ) if _GOGO_EFFECTS_CROSS_SYSROOT else Path("/") / "usr" / "aarch64-linux-gnu"
...
# GoGo Effects Environment Variables
MSYS2_DIR: Path = Path(
    os.environ.get("GOGO_EFFECTS_MSYS2_DIR")
    or (find_msys2_win_dir() or f"{HOMEDRIVE}\\msys64")
)
GIT_PATH: str = os.environ.get("GOGO_EFFECTS_GIT_PATH") or (
    "git"
    if find_program("git") or CURR_PLATFORM != "windows"
    else f"{HOMEDRIVE}\\Program Files\\Git\\cmd\\git.exe"
)
_GOGO_EFFECTS_CROSS_SYSROOT = os.environ.get("GOGO_EFFECTS_CROSS_SYSROOT")

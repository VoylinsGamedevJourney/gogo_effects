#!/usr/bin/env python3
"""
GoGo Effects FFmpeg Builder Script
...
def copy_lib_files_linux(arch: str):
    dest_dir = Path("../test_room/addons/gogo_effects") / "bin" / f"linux_{arch}"
    host, sysroot = get_host_and_sysroot("linux", arch)

    # User defined environment variable takes priority if present
    search_paths = (
        ([Path(_GOGO_EFFECTS_CROSS_SYSROOT)] if _GOGO_EFFECTS_CROSS_SYSROOT else [])
...
def copy_lib_files_windows(arch: str):
    dest_dir = Path("../test_room/addons/gogo_effects") / "bin" / f"windows_{arch}"

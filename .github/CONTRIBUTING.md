# Contributing to GoGo Effects 🤝
Thank you for considering contributing to the GoGo Effects project, all help is greatly appreciated!

This document outlines the guidelines for contributing to the GoGo Effects project. Please take a moment to review the information below, before making your first contribution.

## Ways to contribute
There are many ways you can contribute to the GoGo Effects project:
- Report bugs you find whilst using GoGo Effects;
...
- Spread the word of GoGo Effects!
...
1. Go to the [Issues tab](https://github.com/VoylinsGamedevJourney/gogo_effects/issues);
...
1. Provide a clear and detailed description of the issue, including:
    ...
    * Expected behaviour;
    * Actual behaviour;
    * Version of GoGo Effects you are using;
...
### Project overview
GoGo Effects is an open-source GDExtension to provide a faster and more flexible way to create videos from Godot projects. The supported version of Godot will follow the newest release of Godot with every release from GoGo Effects. The primary languages used are GDScript and C++ (for the interaction with FFmpeg through the GDExtension part). The project is licensed under the GPL-3.0 license, partly due to the use of FFmpeg with the GPL-compatible codecs. GoGo Effects will stay open-source under the GPL license, even if we end up moving away from FFmpeg since most video codec libraries fall under this license anyway.
...
1. **Prerequisites:**
    ...
    * (Optional) Familiarity with C++ and GDExtension if you plan on working on the GDE GoGo Effects part of the editor;
    * (Optional) knowledge of video(/audio) formats and FFmpeg can be helpful;
1. **Fork the repo:**
    * [Fork the GoGo Effects project from GitHub](https://github.com/VoylinsGamedevJourney/gogo_effects/fork) to create your own copy of GoGo Effects to work in;
    * Clone your fork to your local machine
1. **Build the GDE:**
    * Run `build.py` with python3 from the main project folder;
    * Follow the steps to build for your system (choose debug);
1. **Open project in Godot:**
    * Opening the project can be done through launching Godot and opening the project which is inside the `test_room` folder.
...
From there we will review the code and test it ourselves as well. If everything is okay, we merge the PR. If not, we will ask for some changes to happen. Once merged: Congratulations, you've contributed to GoGo Effects!
...
If you want to support GoGo Effects financially, you can do so via:
- **Support me personally via Ko-fi:** This helps support my overall work, including GoGo Effects development, GoZen development, videos for the YT channel, and other projects.
